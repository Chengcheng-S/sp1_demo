// given an ethereum beacon root a start slot an end alot and an eignenpod address returns the 
...
pub fn main() {
    ...
    let eigenpod_address = ExecutionAddress::try_from(hex!(
        "e9cd1419a015dd05d47f6139f5b8e86b1e9e5cdd"
    ).to_vec()).unwrap();
    ...
    let (withdrawal_slots, validator_indexes) = 
        hints::withdrawal_range(beacon_block_root, start_slot, end_slot, &eigenpod_address);
