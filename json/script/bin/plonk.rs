...
fn main() {
    ...
    let mut plonk_proof = client.prove_plonk(&pk, stdin).expect("proving failed");

    // Read output.
    let val = plonk_proof.public_values.read::<String>();
    println!("Value of {} is {}", key, val);

    let account_state = plonk_proof.public_values.read::<Account>();
    println!(
        "Final account state: {}",
